...
if [ "$START_TYPE" = "login" ]; then
    ...
    echo "=== Iniciando LoginServer + GameServer ==="
    ...
    # Configurar properties para ambos
    configure_properties "both" "$SERVER_NUM"
    
    (
      cd login || exit 1
      java $LOGIN_JAVA_OPTS -cp "$CLASSPATH" ext.mods.loginserver.LoginServer
    ) &
    LOGIN_PID=$!
    
    (
      cd game || exit 1
      java $GAME_JAVA_OPTS -cp "$CLASSPATH" ext.mods.gameserver.GameServer "$KEY" "$L2_EMAIL"
    ) &
    GAME_PID=$!
    ...
    wait -n $LOGIN_PID $GAME_PID 2>/dev/null || wait $LOGIN_PID $GAME_PID
    
    echo "Um dos servidores parou. Desligando..."
    shutdown
fi
