...
public class EffectHeal extends AbstractEffect
{
	...
	public boolean onStart()
	{
		...
		final double adjustedPower = BossZergManager.getInstance().applyHealPenalty(getEffector(), getEffected(), power);
		final double amount = getEffected().getStatus().addHp(adjustedPower * getEffected().getStatus().calcStat(Stats.HEAL_EFFECTIVNESS, 100, null, null) / 100.);
