...
import ext.mods.gameserver.scripting.script.event.Squash;
...
public final class Formulas
{
	...
	public static final byte SKILL_COUNTER = 2;
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss)
	{
		...
		final double critDamPosMul = ((attacker.getStatus().calcStat(Stats.CRITICAL_DAMAGE_POS, 1, target, skill) - 1) / 2 + 1);
		final double posMul = getPosMul(attacker, target, true);
	...
	{
		final int patkSpd = attacker.getStatus().getPAtkSpd();
		final boolean isMeleePlayer = attacker instanceof Player && attacker.getStatus().getPhysicalAttackRange() <= 80;
		final int base = isMeleePlayer ? 350000 : 500000;
		return Math.max(100, base / patkSpd) + ((attacker instanceof Npc) ? 300 : 0);
	}
