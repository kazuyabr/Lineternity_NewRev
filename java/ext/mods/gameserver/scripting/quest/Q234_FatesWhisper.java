...
public class Q234_FatesWhisper extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("selectBGrade_"))
		{
			...
			htmltext = getHtmlText("31002-13.htm", player).replace("%weaponname%", WEAPONS.get(st.getInteger("weaponId")));
		}
		else if (event.startsWith("confirmWeapon"))
		{
			st.set("bypass", 1);
			htmltext = getHtmlText("31002-14.htm", player).replace("%weaponname%", WEAPONS.get(st.getInteger("weaponId")));
		}
		else if (event.startsWith("selectAGrade_"))
		{
			if (st.getInteger("bypass") == 1)
			{
				...
				if (player.getInventory().hasItems(itemId))
				{
					...
					htmltext = getHtmlText("31002-12.htm", player).replace("%weaponname%", ItemData.getInstance().getTemplate(aGradeItemId).getName());
					...
					htmltext = getHtmlText("31002-15.htm", player).replace("%weaponname%", WEAPONS.get(itemId));
			}
			else
				...
				switch (npc.getNpcId())
				{
					case 31002:
						...
						else if (cond == 10)
						{
							...
							{
								final int itemId = st.getInteger("weaponId");
								htmltext = getHtmlText((player.getInventory().hasItems(itemId)) ? "31002-AGradeList.htm" : "31002-15.htm", player).replace("%weaponname%", WEAPONS.get(itemId));
							}
							else
								htmltext = "31002-BGradeList.htm";
						}
