...
public class Q335_SongOfTheHunter extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("Tor_list_1"))
		{
			...
			if (st.getInteger("tor") == 0)
			{
				...
				htmltext = getHtmlText("30745-16.htm", player).replace("%reply0%", TOR_REQUEST_1_MENU[reply0]).replace("%reply1%", TOR_REQUEST_1_MENU[reply1]).replace("%reply2%", TOR_REQUEST_1_MENU[reply2]).replace("%reply3%", TOR_REQUEST_1_MENU[reply3]).replace("%reply4%", TOR_REQUEST_1_MENU[reply4]);
			}
		}
		else if (event.equalsIgnoreCase("Tor_list_2"))
		{
			if (st.getInteger("tor") == 0)
			{
				...
				htmltext = getHtmlText("30745-16.htm", player).replace("%reply0%", TOR_REQUEST_2_MENU[reply0]).replace("%reply1%", TOR_REQUEST_2_MENU[reply1]).replace("%reply2%", TOR_REQUEST_2_MENU[reply2]).replace("%reply3%", TOR_REQUEST_2_MENU[reply3]).replace("%reply4%", TOR_REQUEST_2_MENU[reply4]);
			}
		}
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		if (cond == 1)
		{
			...
			else if (npcId == BREKA_ORC_WARRIOR && Rnd.get(100) < 20)
			{
			...
			else if (npcId == WINDSUS && !player.getInventory().hasItems(ALEPH_SKIN) && Rnd.get(100) < 20)
		...
		else
		{
			if (cond == 2 && st.getInteger("grey2") > 0)
			{
				...
				if (dropData != null)
					dropItems(player, dropData[0], 1, dropData[1], dropData[2]);
				else if (npcId == TARLK_BUGBEAR_WARRIOR && Rnd.get(100) < 20)
				{
