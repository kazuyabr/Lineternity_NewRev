...
public class Q105_SkirmishWithTheOrcs extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30218-03.htm"))
		{
			...
			giveItems(player, Rnd.get(1836, 1839), 1);
		}
		return htmltext;
	}
		...
		switch (st.getState())
		{
			...
			case STARTED:
				...
				else if (cond == 2)
				{
					...
					giveItems(player, Rnd.get(1840, 1843), 1);
				}
				else if (cond == 3)
	...
	public void onMyDying(Npc npc, Creature killer)
	{
		...
		switch (npc.getNpcId())
		{
			case KABOO_CHIEF_UOPH, KABOO_CHIEF_KRACHA, KABOO_CHIEF_BATOH, KABOO_CHIEF_TANUKIA:
				if (st.getCond() == 1 && player.getInventory().hasItems(npc.getNpcId() - 25223))
				{
					st.setCond(2);
					playSound(player, SOUND_MIDDLE);
			...
			case KABOO_CHIEF_TUREL, KABOO_CHIEF_ROKO:
				if (st.getCond() == 3 && player.getInventory().hasItems(npc.getNpcId() - 25224))
				{
					st.setCond(4);
					playSound(player, SOUND_MIDDLE);
			...
			case KABOO_CHIEF_KAMUT, KABOO_CHIEF_MURTIKA:
				if (st.getCond() == 3 && player.getInventory().hasItems(npc.getNpcId() - 25225))
				{
					st.setCond(4);
					playSound(player, SOUND_MIDDLE);
					giveItems(player, KABOO_CHIEF_TORC_2, 1);
