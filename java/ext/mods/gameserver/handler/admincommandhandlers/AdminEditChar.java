	...
	public void useAdminCommand(String command, Player player)
	{
		...
		else if (command.startsWith("admin_set"))
		{
			if (!st.hasMoreTokens())
			{
				...
				player.sendMessage("Usage: //set <pvp|pk>");
				return;
			}
			...
			switch (st.nextToken())
			{
				...
				case "pvp":
					try
					{
						...
						final int newPvp = Integer.parseInt(st.nextToken());
						if (newPvp < 0)
						{
							player.sendMessage("The pvp value must be greater or equal to 0.");
							return;
						}
						
						targetPlayer.setPvpKills(newPvp);
						targetPlayer.sendPacket(new UserInfo(targetPlayer));
						
						player.sendMessage("You successfully set " + targetPlayer.getName() + "'s pvp to " + newPvp + ".");
					}
					catch (Exception e)
					{
						player.sendMessage("Usage: //set pvp <number>");
					}
					break;
				...
				case "pk":
					try
					{
						...
						final int newPk = Integer.parseInt(st.nextToken());
						if (newPk < 0)
						{
							player.sendMessage("The pk value must be greater or equal to 0.");
							return;
						}
						
						targetPlayer.setPkKills(newPk);
						targetPlayer.sendPacket(new UserInfo(targetPlayer));
						
						player.sendMessage("You successfully set " + targetPlayer.getName() + "'s pk to " + newPk + ".");
					}
					catch (Exception e)
					{
						player.sendMessage("Usage: //set pk <number>");
					}
					break;
				case "class":
					try
					...
					try
					{
						...
						switch (targetWorldObject)
						{
							case Player targetPlayer:
								...
								if (PlayerInfoTable.getInstance().getPlayerObjectId(newName) > 0 && !targetPlayer.getName().equalsIgnoreCase(newName))
								{
									player.sendPacket(SystemMessageId.INCORRECT_NAME_TRY_AGAIN);
									return;
								}
								
								if (targetPlayer.getName().equalsIgnoreCase(newName))
								{
									final String tempName = newName + "_temp";
									targetPlayer.setName(tempName);
				...
				case "title":
					try
					{
						final String newTitle = st.hasMoreTokens() ? st.nextToken() : "";
