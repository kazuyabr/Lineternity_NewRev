...
public class HeroManager
{
	...
	public void activateHero(Player player)
	{
		...
		String className = player.getActingPlayer().getClassId().getParent().name().toLowerCase().replace('_', ' ');
		
		className = Arrays.stream(className.split(" ")).map(word -> word.substring(0, 1).toUpperCase() + word.substring(1)).collect(Collectors.joining(" "));
		
		_heroMessages.put(player.getObjectId(), className);
