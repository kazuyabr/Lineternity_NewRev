...
public abstract class ClanHallSiege extends Quest implements Siegable
{
	...
	public void endSiege()
	{
	    ...
	    final Clan oldOwner = (_hall.getOwnerId() > 0) ? ClanTable.getInstance().getClan(_hall.getOwnerId()) : null;
		
		if (_missionAccomplished && winner != null)
		{
	        if (oldOwner == null) 
	            winner.addReputationScore(300);
	        else if (!oldOwner.equals(winner))
	            winner.addReputationScore(500);
	    ...
	    else if (oldOwner != null && !_missionAccomplished)
	    {
	        if (oldOwner.getLevel() < 5)
	            oldOwner.takeReputationScore(300);
	        else
	            oldOwner.takeReputationScore(1000);
	    }
		else
